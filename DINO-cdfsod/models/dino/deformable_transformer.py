...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, use_global_token, d_model=256, nhead=8, 
        ...
        if deformable_encoder:
            ...
            if self.two_feature_map:
                num_feature_levels_encoder += 1
            encoder_layer = DeformableTransformerEncoderLayer(d_model, dim_feedforward,
                                                          dropout, activation,
                                                          num_feature_levels_encoder, nhead, enc_n_points, add_channel_attention=add_channel_attention, use_deformable_box_attn=use_deformable_box_attn, box_attn_type=box_attn_type)
        else:
        ...
        self.decoder = TransformerDecoder(use_global_token, decoder_layer, num_decoder_layers, decoder_norm,
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None, image_box=None, image_query=None):
        """
        Input:
        ...
        hs, references = self.decoder(
                ...
                level_start_index=level_start_index, 
                spatial_shapes=spatial_shapes,
                valid_ratios=valid_ratios,tgt_mask=attn_mask, image_box=image_box, image_query=image_query)
        #########################################################
        # End Decoder
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, use_global_token, decoder_layer, num_layers, norm=None, 
        ...
        if self.use_global_token:
            ...
            for i in range(num_layers):
                self.image_query_proj.append(nn.Linear(768, d_model))
                self.image_query_norm.append(nn.LayerNorm(d_model))
        if num_layers > 0:
    ...
    def forward(self, tgt, memory,
                ...
                image_box=None, image_query=None
                ):
        """
        Input:
            - tgt: nq, bs, d_model
...
def build_deformable_transformer(args, use_global_token):
    decoder_query_perturber = None
