...
class DINO(nn.Module):
    ...
    def __init__(self, use_global_token, backbone, transformer, num_classes, num_queries, 
                    aux_loss=False, iter_update=False, two_feature_map=False,
                    query_dim=2, 
        ...
        self.two_feature_map = two_feature_map
        ...
        if self.use_global_token:
            ...
            self.foundation_model, self.preprocess = myclip.load('../pretrained_models/ViT-L-14-336px.pt', device)
            ...
            if self.two_feature_map:
                self.patch_token_proj2 = nn.Sequential(
                    nn.Conv2d(1024, hidden_dim, kernel_size=1),
                    nn.GroupNorm(32, hidden_dim),
                )
                self.dinov2 = dinov2.vit_large()
                checkpoint = torch.load('../pretrained_models/dinov2_vitl14_pretrain.pth', map_location='cpu')
                print("Load pre-trained checkpoint from: %s" % ('../pretrained_models/dinov2_vitl14_pretrain.pth'))
                # load pre-trained model
                msg = self.dinov2.load_state_dict(checkpoint, strict=False)
                print(msg)
                self.dinov2.float().eval()
                self.dinov2_preprocess = dinov2.transform(336)
                for child in self.dinov2.children():
                    for param in child.parameters():
                        param.requires_grad = False
        ###################################################################
        # end global query
    ...
    def get_image_feat(self, imgs, boxes, img_metas):
        ...
        for i in range(len(boxes)):
            ...
            for i, box in enumerate(boxs):
                croped = img.crop(box)
                croped = self.dinov2_preprocess(croped)
        ...
        self.dinov2.eval()
        cls_token, patch_tokens = self.dinov2(preprocessed)
    ...
    def forward(self, samples: NestedTensor, targets:List=None):
        ...
        if self.use_global_token:
            ...
            if self.two_feature_map:
                _, patch_token2 = self.get_image_feat(img_no_normalize, single_image_box, img_metas)
            else:
                patch_token2 = None
        ...
        if self.use_global_token:
            ...
            src = self.patch_token_proj(patch_token1)
            pos_l = self.backbone[1](NestedTensor(src, mask)).to(src.dtype)
            srcs.append(src)
            masks.append(mask)
            poss.append(pos_l)
            ...
            if self.two_feature_map:
                mask = torch.zeros((len(img_metas), 24, 24), device=masks[-1].device, dtype=torch.bool)
                src = self.patch_token_proj2(patch_token2)
                pos_l = self.backbone[1](NestedTensor(src, mask)).to(src.dtype)
                srcs.append(src)
                masks.append(mask)
                poss.append(pos_l)
        ...
        if self.dn_number > 0 or targets is not None:
            num_queries = self.num_queries + self.num_global_token if self.use_global_token else self.num_queries
        ...
        hs, reference, hs_enc, ref_enc, init_box_proposal = self.transformer(srcs, masks, input_query_bbox, poss,input_query_label,attn_mask, image_box=image_box, image_query=image_query)
...
def build_dino(args):
    ...
    transformer = build_deformable_transformer(args, use_global_token)
    ...
    model = DINO(
        ...
        two_feature_map=args.two_feature_map,
        aux_loss=True,
        iter_update=True,
        query_dim=4,
